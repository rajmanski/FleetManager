...
- [x] Instalacja TanStack Query: `npm install @tanstack/react-query`
- [x] Instalacja React Router: `npm install react-router-dom`
- [x] Instalacja Axios (dla HTTP requests): `npm install axios`
- [x] Instalacja UI library (według projektu Figma - w folderze /prototype)
- [x] Konfiguracja TanStack Query provider w App.tsx:
  ```tsx
...
- [x] Utworzenie frontend/Dockerfile:
  ```dockerfile
  FROM node:18-alpine
  WORKDIR /app
  COPY package*.json ./
  ...
  ```
- [x] Konfiguracja serwisu frontend w docker-compose.yml:
  ```yaml
  frontend:
    build: ./frontend
    ports:
...
- [x] Utworzenie src/services/api.ts:
  ```ts
...
- [x] Utworzenie tabeli `users` (id, login, password_hash, role, email, created_at, updated_at)
- [x] Utworzenie tabeli `roles` (id, name, description) - OPCJONALNE jeśli role są hardcoded
- [x] Dodanie indeksów (unique na login, index na role)
- [x] Dodanie constraints (NOT NULL, check dla role ENUM)
...
- [x] Utworzenie funkcji do hashowania hasła przy rejestracji
- [x] Utworzenie funkcji do weryfikacji hasła przy logowaniu
- [x] Użycie bcrypt z odpowiednim cost factor (min. 10)
...
- [x] Utworzenie POST /api/v1/auth/login
- [x] Walidacja danych wejściowych (login + hasło niepuste)
- [x] Weryfikacja credentials w bazie danych
- [x] Implementacja blokady konta po 3 nieudanych próbach (REQ-AUTH-003)
...
### E1.3a: Refaktoryzacja auth do układu feature-first
- [x] Przenieść kod auth (handler, service, modele, repozytorium) do jednego modułu `backend/internal/auth`
- [x] Zachować `sqlc` jako źródło zapytań i kodu dostępu do DB
- [x] Zaktualizować wiring w `cmd/server/main.go`
- [x] Zweryfikować działanie przez `go test ./...`

**API:** bez zmian względem E1.3  
**Dane wejściowe:** istniejąca implementacja auth  
**Dane wyjściowe:** ta sama funkcjonalność w nowym układzie katalogów  
**Edge cases:** brak regresji endpointu logowania

---

### E1.3b: Refaktoryzacja auth do układu adapterów (http/application/domain/mysql)
- [x] Rozdzielić auth na warstwy: `internal/adapters/http`, `internal/application/auth`, `internal/domain/auth`, `internal/adapters/mysql`
- [x] Przenieść wygenerowany kod sqlc do `internal/adapters/mysql/sqlc`
- [x] Zachować bez zmian kontrakt endpointu `POST /api/v1/auth/login`
- [x] Zaktualizować wiring w `cmd/server/main.go`
- [x] Zweryfikować działanie przez `go test ./...`

**API:** bez zmian względem E1.3  
**Dane wejściowe:** istniejąca implementacja auth  
**Dane wyjściowe:** ta sama funkcjonalność w układzie adapterowym  
**Edge cases:** brak regresji endpointu logowania

---

### E1.3c: Uproszczenie do docelowego wzorca (feature module + shared sqlc)
- [x] Przenieść logikę auth do spójnego modułu `internal/auth` (handler, service, structy, interfejs repozytorium)
- [x] Pozostawić adapter DB w `internal/adapters/mysql`
- [x] Przenieść wygenerowany kod sqlc do `internal/db/sqlc`
- [x] Zaktualizować wiring w `cmd/server/main.go`
- [x] Zweryfikować działanie przez `go test ./...`

**API:** bez zmian względem E1.3  
**Dane wejściowe:** istniejąca implementacja auth  
**Dane wyjściowe:** ten sam endpoint i zachowanie w uproszczonej strukturze  
**Edge cases:** brak regresji endpointu logowania

---

### E1.3d: Rename warstwy adaptera DB na repository
- [x] Przenieść implementację auth repozytorium z `internal/adapters/mysql` do `internal/repository`
- [x] Zaktualizować importy i wiring w `cmd/server/main.go`
- [x] Zachować bez zmian kontrakt endpointu `POST /api/v1/auth/login`
- [x] Zweryfikować działanie przez `go test ./...`

**API:** bez zmian względem E1.3  
**Dane wejściowe:** istniejąca implementacja auth  
**Dane wyjściowe:** ten sam endpoint i zachowanie przy czytelniejszym nazewnictwie warstwy  
**Edge cases:** brak regresji endpointu logowania

---

### E1.4: Implementacja JWT
- [x] Utworzenie funkcji generującej JWT (payload: userID, role, exp)
- [x] Utworzenie middleware weryfikującego JWT przy każdym zapytaniu
- [x] Obsługa wygasłych tokenów (401 Unauthorized)
- [x] Określenie czasu ważności tokenu (BRAKUJĄCA DECYZJA - domyślnie 24h?)
...
- [x] Generowanie refresh token (JWT z dłuższym czasem ważności: 7 dni)
- [x] Przechowywanie refresh token w httpOnly cookie
- [x] Endpoint POST /api/v1/auth/refresh (odświeżanie access token)
...
- [x] Axios interceptor dla 401 Unauthorized:
  - Wywołanie POST /api/v1/auth/refresh
  - Jeśli sukces: retry original request z nowym tokenem
  - Jeśli fail: redirect do /login
- [x] Timer dla proaktywnego odświeżania (5 min przed wygaśnięciem)
- [x] Wyświetlenie komunikatu przy wylogowaniu z powodu wygasłej sesji

---

### E1.5b: Backend API - logika blokowania konta
- [x] W endpoint POST /api/v1/auth/login dodać:
...
- [x] Endpoint PUT /api/v1/users/{id}/unlock (tylko Administrator)
- [x] Ustawienie:
  - failed_login_attempts = 0
  - locked_until = NULL
- [x] Log w Changelogger: "Account manually unlocked by admin" (POMINIĘTE w tej iteracji zgodnie z decyzją A / Q1.5)
...
- [x] W liście użytkowników (E1.8) dodać kolumnę "Status"
- [x] Ikona 🔒 dla zablokowanych kont (locked_until > NOW())
- [x] Tooltip pokazujący "Locked until {timestamp}"
- [x] Przycisk "Odblokuj" dla Administratora (wywołanie E1.6b)
- [x] Confirmation dialog przed odblokowaniem

---

### E1.5: Middleware autoryzacji RBAC
- [x] Utworzenie middleware sprawdzającego uprawnienia na podstawie roli
- [x] Implementacja macierzy uprawnień zgodnie z tabelą 5.1
- [x] Zwracanie 403 Forbidden przy braku uprawnień
...
- [x] GET /api/v1/admin/users (lista wszystkich użytkowników)
- [x] GET /api/v1/admin/users/{id} (szczegóły użytkownika)
- [x] POST /api/v1/admin/users (utworzenie nowego użytkownika)
- [x] PUT /api/v1/admin/users/{id} (edycja użytkownika)
- [x] DELETE /api/v1/admin/users/{id} (usunięcie użytkownika - soft delete?)

**API:** Endpoints opisane powyżej  
**Autoryzacja:** Tylko rola Administrator  
**Edge cases:**  
- Czy można usunąć samego siebie? (NIE - 403)
- Soft delete czy hard delete? (SOFT DELETE przez is_active=0)
- Czy można zmienić hasło innego użytkownika? (NIE - 400)

---

### E1.7: Frontend - strona logowania
- [x] Formularz logowania (login, hasło)
- [x] Obsługa błędów (nieprawidłowe credentials, konto zablokowane)
- [x] Zapisanie tokenu JWT w localStorage/sessionStorage
...
- [x] Strona z listą użytkowników
- [x] Formularz dodawania użytkownika
- [x] Formularz edycji użytkownika
- [x] Potwierdzenie przed usunięciem użytkownika

---

### E1.9: Wylogowywanie użytkownika
- [x] POST /api/v1/auth/logout (backend) - unieważnienie sesji przez wyczyszenie cookie refresh tokena
- [x] Przycisk "Logout" w interfejsie (frontend)
- [x] Usunięcie access tokena z localStorage/sessionStorage po wylogowaniu
- [x] Przekierowanie użytkownika na stronę logowania po wylogowaniu
- [x] Obsługa wylogowania przy wygasłej/nieobecnej sesji (idempotentne zachowanie)

**API:** `POST /api/v1/auth/logout`  
**Dane wejściowe:** refresh token cookie (jeśli obecne)  
**Dane wyjściowe:** 200 OK + informacja o poprawnym wylogowaniu  
**Edge cases:**  
- Wylogowanie przy braku refresh tokena (nie zwracać 500)
- Wiele kart przeglądarki (spójne czyszczenie stanu po stronie FE)
...
- [x] Utworzenie tabeli `vehicles` (id, vin, make, model, year, mileage, status, created_at, updated_at)
- [x] Dodanie unique constraint na VIN
- [x] Dodanie ENUM dla status (Available, In_Use, In_Service, Decommissioned - ZAŁOŻENIE)
- [x] Dodanie indeksów (unique na VIN, index na status)
- [x] Stworzenie madrego sposóbu na uruchamianie migracji
...
- [x] Implementacja funkcji walidującej strukturę VIN (17 znaków, checksum)
- [x] Walidacja przy dodawaniu/edycji pojazdu
...
- [x] GET /api/v1/vehicles (lista pojazdów z filtrowaniem i paginacją)
- [x] GET /api/v1/vehicles/{id} (szczegóły pojazdu)
- [x] POST /api/v1/vehicles (utworzenie pojazdu)
- [x] PUT /api/v1/vehicles/{id} (edycja pojazdu)
- [x] DELETE /api/v1/vehicles/{id} (usunięcie pojazdu)
...
- [x] PATCH /api/v1/vehicles/{id}/status (zmiana statusu np. "In_Service")
- [x] Walidacja przejść między statusami (BRAK SPECYFIKACJI - ZAŁOŻENIE: dowolne)
...
### E2.5 Backend API - soft delete pojazdu
- [x] Modyfikacja DELETE /api/v1/vehicles/{id}:
...
### E2.6: Backend API - restore usuniętego pojazdu
- [x] Endpoint PUT /api/v1/vehicles/{id}/restore (tylko Administrator)
- [x] Ustawienie deleted_at = NULL
- [x] Sprawdzenie czy VIN nie koliduje z innym active vehicle

**Autoryzacja:** Tylko Administrator

---

### E2.7: Frontend - filtrowanie usuniętych pojazdów
- [x] W liście pojazdów (E2.5) dodać checkbox "Pokaż usunięte" (tylko Administrator)
- [x] Domyślnie: WHERE deleted_at IS NULL
- [x] Jeśli zaznaczone: pokazuj wszystkie
- [x] Oznaczenie usuniętych (szary kolor, ikona 🗑️)
- [x] Przycisk "Przywróć" dla usuniętych (tylko Administrator)

---

### E2.8: Frontend - lista pojazdów
- [x] Tabela z listą pojazdów (VIN, marka, model, przebieg, status)
- [x] Filtrowanie po statusie
- [x] Wyszukiwanie po VIN/marce
- [x] Paginacja

---

### E2.9: Frontend - formularz dodawania/edycji pojazdu
- [x] Pola: VIN, marka, model, rok produkcji, przebieg
- [x] Walidacja VIN po stronie frontendu
- [x] Obsługa błędów (duplikat VIN, nieprawidłowa walidacja)

---

### E2.10: Frontend - widok szczegółów pojazdu
- [x] Wyświetlenie wszystkich danych pojazdu
- [x] Historia przebiegu (jeśli dostępna)
- [x] Przyciski edycji/usunięcia (z uwzględnieniem uprawnień)

### E2.11: Frontend i Backend - usprawnienia
- [x] Endpoint delete nie działa - po kliknięciu delete behicle dostaje - internal server error.
- [x] W formularzu brakuje capacity oraz plate number
- [x] Edycja czasów - teraz na frontendzie wyglądajątak: 2026-02-17T15:17:58Z powinnybyć ładnie sformatowane
- [x] Formularze mają używać useForm tak jak w przypadku users!

### E2.12: Frontend - refactoring oraz usprawnienia
- [x] Podzielenie kodu na komponenty - mają być generyczne aby z nich budować na następnych etapach z których bedziemy korzystać
- [x] Podział na hooki tam gdzie ma to sens
- [x] Frontend powinien być zgodny z best practices dla projektu w React - najpierw skonsultuj ze mnąrozwiązanie
- [x] Zamieszczenie infromacji w /docs o tym jak ma być nasz frontend przez Ciebie imeplementowany - co gdzie i jakie narzędzie aby uniknąć refactoringu w przyszłości 
- [x] Weryfikacja czy wszystko nadal działa
...
- [x] Utworzenie tabeli `drivers` (id, first_name, last_name, pesel, phone, email, status, created_at, updated_at)
- [x] Dodanie unique constraint na PESEL
- [x] Dodanie ENUM dla status (Available, OnLeave, InRoute - zgodnie z REQ-DRV-005)
- [x] Szyfrowanie kolumny PESEL (RODO)
...
- [x] Implementacja funkcji walidującej PESEL (11 cyfr + checksum)
- [x] Walidacja przy dodawaniu/edycji kierowcy
...
- [x] GET /api/v1/drivers (lista kierowców)
- [x] GET /api/v1/drivers/{id} (szczegóły kierowcy)
- [x] POST /api/v1/drivers (utworzenie kierowcy)
- [x] PUT /api/v1/drivers/{id} (edycja kierowcy)
- [x] DELETE /api/v1/drivers/{id} (archiwizacja kierowcy?)
...
### E3.4: Implementacja szyfrowania PESEL (AES-256)
- [x] Utworzenie pakietu internal/crypto z funkcjami:
...
### E3.5: Wyszukiwanie po PESEL
- [x] Query builder dla wyszukiwania:
...
- [x] Dla małej liczby rekordów (<1000) akceptowalne

**Uwaga:** To nie jest optymalne dla dużej liczby rekordów, ale akceptowalne dla MVP (kilkudziesięciu kierowców).

---

### E3.6: Backend API - soft delete kierowcy
- [x] Modyfikacja DELETE /api/v1/drivers/{id}:
...
### E3.7: Backend API - odzyskiwanie usuniętego kierowcy
- [x] Endpoint PUT /api/v1/drivers/{id}/restore (tylko Administrator)
- [x] Ustawienie deleted_at = NULL
- [x] Sprawdzenie czy PESEL nie koliduje z innym active driver

---

### E3.8: Frontend - filtrowanie usuniętych kierowców
- [x] W liście kierowców (E3.5) dodać checkbox "Pokaż usunięte"
- [x] Domyślnie: pokazuj tylko WHERE deleted_at IS NULL
- [x] Jeśli checkbox zaznaczony: pokazuj wszystkie
- [x] Oznaczenie usuniętych (szary kolor, ikona 🗑️)
- [x] Przycisk "Przywróć" dla usuniętych (tylko Administrator)

---

### E3.9: Backend API - sprawdzanie dostępności kierowcy
- [x] GET /api/v1/drivers/{id}/availability?date=YYYY-MM-DD
- [x] Sprawdzenie czy kierowca ma przypisane zlecenie w danym okresie
...
### E3.10: Frontend - lista kierowców
- [x] Tabela z listą kierowców (imię, nazwisko, PESEL zaszyfrowany?, status)
- [x] Filtrowanie po statusie
- [x] Wyszukiwanie po imieniu/nazwisku

---

### E3.11: Frontend - formularz dodawania/edycji kierowcy
- [x] Pola: imię, nazwisko, PESEL, telefon, email
- [x] Walidacja PESEL po stronie frontendu
- [x] Obsługa błędów (duplikat PESEL)
...
- [x] Sprawdzenie czy kolumny już istnieją w Drivers (powinny być z migracji init):
...
- [x] Jeśli brakuje, dodać przez migrację
...
- [x] Rozszerzenie PUT /api/v1/drivers/{id} o możliwość edycji pól certyfikatów
- [x] Walidacja dat (expiry_date >= current_date przy ustawianiu)
- [x] Walidacja że adr_expiry_date może być ustawiona tylko gdy adr_certified=1
...
- [x] Funkcja service layer: `CanDriverTransportHazardousCargo(driver_id, order_id) bool`
- [x] Sprawdzenie czy kierowca ma adr_certified=1
- [x] Sprawdzenie czy adr_expiry_date >= NOW() lub IS NULL (akceptujemy brak daty)
- [x] Sprawdzenie czy zlecenie zawiera ładunek typu Hazardous (JOIN z Cargo)
- [x] Użycie tej funkcji w E7 (Trips) przed przypisaniem
...
- [x] Funkcja wywoływana przez scheduler (E14): `CheckExpiringCertificates()`
- [x] SELECT na Drivers WHERE:
  - `license_expiry_date BETWEEN NOW() AND NOW() + INTERVAL 30 DAY` OR
  - `adr_expiry_date BETWEEN NOW() AND NOW() + INTERVAL 30 DAY`
- [x] Dla każdego znalezionego kierowcy:
...
- [x] W formularzu kierowcy (E3.6) dodać sekcję "Uprawnienia":
...
- [x] Walidacja po stronie frontendu:
...
- [x] W liście kierowców (E3.5) dodać kolumnę "Status uprawnień"
- [x] Ikony/kolory:
  - ✅ Zielony: Wszystkie certyfikaty ważne
  - ⚠️ Żółty: Certyfikat wygasa w ciągu 30 dni
  - ❌ Czerwony: Certyfikat wygasły
- [x] Tooltip pokazujący szczegóły (która data, kiedy wygasa)
- [x] Filtr: "Pokaż tylko kierowców z ważnymi certyfikatami"
...
- [x] Uzyskanie API key
- [x] Konfiguracja dozwolonych domen
- [x] Ustawienie limitów zapytań (monitoring kosztów)

**Dane wyjściowe:**
- Dokumentacja: `docs/GOOGLE_MAPS_SETUP.md`
- Zmienne środowiskowe: `GOOGLE_MAPS_API_KEY`, `VITE_GOOGLE_MAPS_API_KEY` w `.env.example` i config backendu
...
- [x] POST /api/v1/routes/geocode
- [x] Wywołanie Google Maps Geocoding API
- [x] Zwrócenie współrzędnych (lat, lng)
...
- [x] POST /api/v1/routes/calculate
- [x] Wywołanie Google Maps Directions API
- [x] Obliczenie dystansu, czasu przejazdu
- [x] Obsługa waypoints (punkty pośrednie)
...
- [x] Integracja Google Maps JavaScript API
- [x] Wyświetlenie mapy
- [x] Oznaczenie punktów (załadunek, rozładunek, waypoints)
- [x] Rysowanie trasy (polyline)

---

### E5.5: Frontend - formularz planowania trasy
- [x] Pole adres załadunku (autocomplete z Google Places)
- [x] Pole adres rozładunku
- [x] Możliwość dodania waypoints (dynamiczna lista)
- [x] Przycisk "Oblicz trasę"
- [x] Wyświetlenie wyniku (dystans, czas)

---

### E5.6: Backend API - CRUD RouteWaypoints
- [x] GET /api/v1/routes/{route_id}/waypoints (lista waypoints dla trasy)
- [x] POST /api/v1/routes/{route_id}/waypoints (dodanie waypoint)
- [x] PUT /api/v1/waypoints/{id} (edycja waypoint - adres, kolejność)
- [x] DELETE /api/v1/waypoints/{id} (usunięcie waypoint)
- [x] PATCH /api/v1/waypoints/reorder (zmiana kolejności - bulk update sequence_order)
...
- [x] Trigger lub service function po DELETE waypoint:
  - UPDATE RouteWaypoints SET sequence_order = sequence_order - 1 WHERE route_id=... AND sequence_order > deleted_sequence
- [x] Walidacja ciągłości przy INSERT/UPDATE

---

### E5.8: Frontend - zarządzanie waypoints z drag-and-drop
- [x] W formularzu planowania trasy (E5.5) dodać sekcję "Punkty pośrednie"
- [x] Lista waypoints z możliwością:
...
- [x] Walidacja: max 10 waypoints
- [x] Numeracja automatyczna (1, 2, 3...)
- [x] Przycisk "Dodaj punkt pośredni"
...
---
### E5.9: Frontend - refactoring epica
...
- [x] Utworzenie tabeli `orders` (id, client_id, cargo_type, cargo_weight_kg, cargo_volume_m3, price, status, created_at, updated_at)
- [x] Dodanie foreign key do `clients` (BRAK W WYMAGANIACH - ZAŁOŻENIE: trzeba dodać tabelę clients)
- [x] Dodanie ENUM dla status (Created, In_Progress, Completed, Cancelled - ZAŁOŻENIE)
...
- [x] GET /api/v1/clients (lista klientów z paginacją)
- [x] GET /api/v1/clients/{id} (szczegóły klienta)
- [x] POST /api/v1/clients (utworzenie klienta)
- [x] PUT /api/v1/clients/{id} (edycja klienta)
- [x] DELETE /api/v1/clients/{id} (soft delete - ustawienie deleted_at)
...
- [x] Funkcja `ValidateNIP(nip string) (bool, error)`
- [x] Sprawdzenie długości (10 cyfr)
- [x] Sprawdzenie czy tylko cyfry
- [x] Walidacja checksum NIP (algorytm REGON/NIP)
- [x] Użycie w POST i PUT /api/v1/clients`
...
- [x] PUT /api/v1/clients/{id}/restore (tylko Administrator)
- [x] Ustawienie deleted_at = NULL
- [x] Sprawdzenie czy NIP nie koliduje z innym active client
...
- [x] Tabela z kolumnami:
  ...
  - Email kontaktowy
  - Akcje (Edytuj, Usuń)
- [x] Wyszukiwanie po nazwie lub NIP
- [x] Paginacja (50 per page)
- [x] Filtr: "Pokaż usunięte" (tylko Administrator)
...
- [x] W formularzu zlecenia (E6.11) pole wyboru klienta
- [x] Autocomplete z wyszukiwaniem (debounce 300ms):
  - GET /api/v1/clients?search={query}&limit=10
  - Wyszukiwanie po company_name i NIP
- [x] Wyświetlanie: "Nazwa firmy (NIP: XXX-XXX-XX-XX)"
- [x] Możliwość szybkiego dodania nowego klienta (przycisk "+", modal)
...
- [x] Utworzenie tabeli `clients` (id, name, nip, address, phone, email, created_at)
- [x] Dodanie unique constraint na NIP
...
- [x] GET /api/v1/orders (lista zleceń z filtrowaniem)
- [x] GET /api/v1/orders/{id} (szczegóły zlecenia)
- [x] POST /api/v1/orders (utworzenie zlecenia)
- [x] PUT /api/v1/orders/{id} (edycja zlecenia)
- [x] DELETE /api/v1/orders/{id} (usunięcie/anulowanie zlecenia)

**Autoryzacja:** Spedytor (R/W), Administrator (R/W)  

---

### E6.8: Backend API - wybór klienta
- [x] GET /api/v1/clients (lista klientów - autocomplete)
- [x] GET /api/v1/clients/{id} (szczegóły klienta)
...
### E6.9: Frontend - lista zleceń
- [x] Tabela z listą zleceń (ID, klient, typ ładunku, status)
- [x] Filtrowanie po statusie
- [x] Wyszukiwanie po kliencie

---

### E6.10: Frontend - formularz tworzenia zlecenia
- [x] Wybór klienta (autocomplete)
- [x] Pola: typ ładunku, waga, objętość, cena
- [x] Przycisk "Utwórz zlecenie"


### E6.11: Backend API - CRUD ładunków (Cargo)
- [x] GET /api/v1/orders/{order_id}/cargo (lista ładunków dla zlecenia)
- [x] POST /api/v1/orders/{order_id}/cargo (dodanie ładunku do zlecenia)
- [x] PUT /api/v1/cargo/{id} (edycja ładunku)
- [x] DELETE /api/v1/cargo/{id} (usunięcie ładunku ze zlecenia)
...
### E6.12: Backend API - przypisanie ładunku do waypoint
- [x] PUT /api/v1/cargo/{id}/assign-waypoint
- [x] Walidacja że waypoint_id należy do trasy tego zlecenia
- [x] Ustawienie destination_waypoint_id w Cargo
...
### E6.13: Frontend - zarządzanie ładunkami w zleceniu
- [x] W formularzu zlecenia (E6.6) dodać sekcję "Ładunki"
- [x] Lista ładunków z możliwością dodania/usunięcia
- [x] Dla każdego ładunku:
  - Input: Opis
  - Input: Waga (kg)
  - Input: Objętość (m³)
  - Select: Typ (General/Refrigerated/Hazardous)
  - Select: Punkt rozładunku (opcjonalny, z waypoints)
- [x] Przycisk "+ Dodaj ładunek"
- [x] Suma wag na dole (pomocnicze)
...
### E6.14: Frontend - przypisywanie ładunków do waypoints
- [x] W formularzu ładunku (E6.9) dodać select "Punkt rozładunku"
- [x] Lista waypoints z trasy (tylko te z action_type='Dropoff' lub 'Stopover')
- [x] Opcja "Brak konkretnego punktu" (destination_waypoint_id = NULL)
- [x] Pokazanie na mapie które ładunki jadą do którego waypoint (kolory?)

---

### E6.15: Backend API - walidacja capacity przy tworzeniu Trip
- [x] W service layer przy CREATE Trip:
...
- [x] Komunikat błędu: "Total cargo weight (X kg) exceeds vehicle capacity (Y kg)"

---

### E6.16: Refactoring
- [x] Refactoring FE według best practices – dzielenie na hooki, osobne komponenty tam gdzie to ma sens
- [x] Refactoring BE – pominięty (duplikacja parseIDParam minimalna; bez overengineeringu)

**Wykonane (FE):**
- utils/price.ts – formatPrice (OrderDetailPage, OrdersTable)
- utils/nip.ts – formatNipDisplay, normalizeNipDigits (ClientFormModal, ClientAutocompleteInput)
- components/orders/OrderStatusBadge – wspólny badge statusu (OrdersTable, OrderDetailPage)
- schemas/clients.ts – Zod dla ClientFormModal (zgodnie z FRONTEND.md)
- useMutationCallbacks dla assignWaypointMutation (OrderDetailPage)
- **Komponenty generyczne** – Input, Select, Textarea, Checkbox, AutocompleteInput w components/ui; refaktoryzacja formularzy (ClientFormModal, DriverFormModal, VehicleFormModal, UserFormFields, LoginPage, OrderFormModal, CargoItemRow, OrderDetailPage, ClientAutocompleteInput)
- Aktualizacja FRONTEND.md (utils, schemas, komponenty generyczne)
---
...
- [x] GET /api/v1/assignments (lista wszystkich przypisań)
- [x] GET /api/v1/assignments?active=true (tylko aktywne, WHERE assigned_to IS NULL)
- [x] POST /api/v1/assignments (przypisanie kierowcy do pojazdu)
- [x] PUT /api/v1/assignments/{id}/end (zakończenie przypisania, ustawienie assigned_to)
...
- [x] GET /api/v1/vehicles/{id}/assignment-history (historia przypisań pojazdu)
- [x] GET /api/v1/drivers/{id}/assignment-history (historia przypisań kierowcy)
...
- [x] GET /api/v1/vehicles/{id}/availability?date_from=YYYY-MM-DD&date_to=YYYY-MM-DD
- [x] Sprawdzenie statusu pojazdu (Available, nie Service ani Inactive)
- [x] Sprawdzenie czy pojazd nie ma aktywnego Trip w tym okresie
...
- [x] GET /api/v1/drivers/{id}/availability?date_from=YYYY-MM-DD&date_to=YYYY-MM-DD
- [x] Sprawdzenie statusu kierowcy (Available, nie OnLeave)
- [x] Sprawdzenie czy kierowca nie ma aktywnego Trip
...
- [x] GET /api/v1/trips (lista wszystkich przejazdów)
- [x] GET /api/v1/trips?status=Active (filtrowanie po statusie)
- [x] GET /api/v1/trips/{id} (szczegóły przejazdu)
- [x] POST /api/v1/trips (utworzenie przejazdu)
- [x] PATCH /api/v1/trips/{id}/start (rozpoczęcie przejazdu)
- [x] PATCH /api/v1/trips/{id}/finish (zakończenie przejazdu)
- [x] PATCH /api/v1/trips/{id}/abort (przerwanie przejazdu)
...
- [x] PATCH /api/v1/trips/{id}/start
- [x] Walidacja że status='Scheduled'
- [x] Ustawienie start_time=NOW()
- [x] Zmiana status='Active'
...
- [x] PATCH /api/v1/trips/{id}/finish
- [x] Walidacja że status='Active'
- [x] Ustawienie end_time=NOW()
- [x] Zmiana status='Finished'
- [x] Wymagane: actual_distance_km (z body)
...
- [x] Modal/strona "Utwórz przejazd" dla zlecenia
- [x] Wybór pojazdu:
...
- [x] DateTimePicker: Planowany start
- [x] Podsumowanie:
  - Trasa (start → waypoints → end)
  - Ładunki (lista)
  - Suma wag vs capacity
- [x] Przycisk "Utwórz przejazd"
...
- [x] Filtrowanie po statusie
- [x] Wyszukiwanie po VIN / nazwisku kierowcy / order_number
- [x] Sortowanie po dacie

---

### E7.10: Frontend - szczegóły Trip i akcje
- [x] Widok szczegółów Trip (modal lub strona)
- [x] Sekcje:
...
- [x] Przyciski akcji (zależne od statusu):
...
### E7.12: Frontend - walidacja przy przypisywaniu kierowcy do Hazardous cargo
